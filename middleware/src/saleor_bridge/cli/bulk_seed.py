"""CLI: bulk seed каталога Odoo → Saleor (ADR-0013).

    python -m saleor_bridge.cli.bulk_seed bulk-seed
    python -m saleor_bridge.cli.bulk_seed bulk-seed --dry-run
    python -m saleor_bridge.cli.bulk_seed wipe --yes        # снести Saleor-каталог
"""
...
app = typer.Typer(add_completion=False, help="Odoo → Saleor catalog seed (Phase 3.2).")
console = Console()
...
def bulk_seed(dry_run: bool = typer.Option(False, "--dry-run", help="План без изменений.")) -> None:
    """Засеять весь каталог Odoo в Saleor (идемпотентно)."""
...
@app.command("wipe")
def wipe(yes: bool = typer.Option(False, "--yes", help="Подтвердить удаление без запроса.")) -> None:
    """DESTRUCTIVE: удалить ВСЕ products + categories в Saleor."""
    if not yes:
        confirm = typer.confirm("Удалить ВСЕ products и categories в Saleor?")
        if not confirm:
            raise typer.Abort()
    settings = get_settings()
    ...
    async def _run() -> dict:
        ...
        # очистить outbound-биндинги, иначе bulk-seed уйдёт в update по мёртвым id
...
def retry_failed() -> None:
    """Re-sync всех saleor.binding со state='failed' (outbound)."""
    settings = get_settings()
...
def bulk_seed_stocks() -> None:
    """Засеять остатки всех активных вариантов Odoo → Saleor (Phase 3.3, идемпотентно)."""
    settings = get_settings()
...
def bulk_seed_variants() -> None:
    """Migrate существующие single-variant продукты → variant bindings (Phase 3.5, ADR-0025).

    Реконсилит набор вариантов каждого синканного шаблона: усыновляет dummy-вариант
    (Phase 3.2) и создаёт binding product.product → Saleor variant. Идемпотентно.
    """
    settings = get_settings()
...
def reconcile_stocks(
    apply: bool = typer.Option(False, "--apply", help="Починить дрейф (иначе dry-run)."),
) -> None:
    """Сверить остатки Odoo vs Saleor (ADR-0018). По умолчанию dry-run.

    Exit 1 при дрейфе (dry-run) или при ошибке fix (--apply); иначе 0.
    """
    settings = get_settings()
