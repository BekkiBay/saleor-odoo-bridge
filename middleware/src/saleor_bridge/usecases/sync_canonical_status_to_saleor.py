"""Usecase: push the canonical justix_status (computed in Odoo) → Saleor order
...
async def sync_canonical_status_to_saleor(
    ...
    status = await so_adapter.fetch_justix_status(odoo, odoo_order_id)
    if not status:
        return SyncResult(ok=True, odoo_id=odoo_order_id, message="no justix_status, skipped")
    ...
    res = await om.update_order_metadata(client, saleor_id, {"justix_status": status})
    if res.ok:
    ...
    return SyncResult(ok=res.ok, odoo_id=odoo_order_id, message=f"justix_status={status}")
