...
async def manifest(settings: Settings = Depends(get_settings)) -> dict:
    """Возвращает App Manifest с подставленным public URL.

    Saleor запросит этот endpoint при `appInstall` mutation.
    Reference: https://docs.saleor.io/developer/extending/apps/architecture/manifest
    """
    m = AppManifest.build(public_url=settings.middleware_public_url)
