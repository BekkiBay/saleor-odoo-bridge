"""Integration-lite: sync_stock_to_saleor with FakeOdoo + respx-mocked Saleor.

Verifies the full flow: variant→template binding → Saleor product → ensure warehouse
→ trackInventory=True → productVariantStocksUpdate with the correct quantity (raw - buffer).
"""
...
def _saleor_router(request: httpx.Request) -> httpx.Response:
    ...
    if "product(id:$id)" in q:
        return httpx.Response(200, json={"data": {"product": {
            "id": _PRODUCT_ID, "name": "Test", "metafields": {},
...
def _odoo() -> FakeOdoo:
    return FakeOdoo(
        variants={5: {"default_code": "SKU-005", "product_tmpl_id": [5, "Test"]}},
        quants=[{"quantity": 20.0, "product_id": [5, "Test"]}],
...
async def test_stock_sync_pushes_buffered_quantity():
    ...
    # productVariantStocksUpdate went out with quantity = 20 - 1 = 19, to the right warehouse/variant
    ...
    # warehouse binding was created, last_sync_out updated
...
async def test_missing_catalog_binding_raises_for_retry():
    ...
    odoo = _odoo()
    odoo.bindings = {}  # catalog hasn't synced the product yet
    client = SaleorClient(api_url=_URL, app_token="t")
